...
wboot.std.ipw.panel.archive <- function(nn, n, deltaY, D, int.cov, i.weights){
  #-----------------------------------------------------------------------------
  v <- stats::rexp(n)
  #v <- v / mean(v)
  ...
  ps.b <- suppressWarnings(stats::glm(D ~ int.cov, family = "binomial", weights = b.weights)$fitted.values)
