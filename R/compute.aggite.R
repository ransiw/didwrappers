#' @title Compute aggregated treatment effect parameters from the unit-time DiDs
#'
#' @description Does the heavy lifting on computing aggregated unit-time
...
compute.aggite <- function(MP,
  ...
  MP_i$DIDparams_i$clustervars <- clustervars
  MP_i$DIDparams_i$bstrap <- bstrap
  MP_i$DIDparams_i$biters <- biters
  MP_i$DIDparams_i$alp <- alp
  MP_i$DIDparams_i$cband <- cband
  dp <- MP$DIDparams


  if(!(type %in% c("simple", "dynamic", "group","unit", "calendar", cohortnames))) {
    stop('`type` must be one of c("simple", "dynamic", "group", "unit", "calendar", "or custom cohort")')
  }
  ...
  if(na.rm){
    ...
    # If aggte is of the unit type, ensure we have non-missing post-treatment ATTs for each unit
    if(type == "unit"){
  ...
  # If aggte is of the custom cohort type, ensure we have non-missing post-treatment ATTs for each cohort
  if(type %in% c(cohortnames)){
    idcohort = unique(data[,c(idname,type)])
    has_multiple_types <- any(duplicated(idcohort[,idname]) | duplicated(idcohort[,idname], fromLast = TRUE))
    if (has_multiple_types){
      stop("Some ids belong to more than one cohort elected in `type`. Remove any duplicates before proceeding")
    }
    idcohortlong = data.frame(id)
    colnames(idcohortlong) = c(idname)
    idcohortlong = merge(idcohortlong, idcohort, by = idname)
    cohort = idcohortlong[,type]
    glist <- sort(unique(cohort))
    # Get the groups that have some non-missing ATT(g,t) in post-treatmemt periods
    gnotna <- sapply(glist, function(g) {
      # look at post-treatment periods for group g
      whichg <- which((id == g) & (group <= t))
  ...
  #-----------------------------------------------------------------------------
  # Compute the unit (i.e., selective) treatment timing estimators
  #-----------------------------------------------------------------------------
